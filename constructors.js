...
// Code here
User.prototype.removeSavedPosts = function (id) {
  const remove = this.savedPosts.findIndex(id => this.savedPosts.id === newSavedPost.id);
...
User.prototype.changePostRating = function(id, newRating){
  const id = this.savedPosts.findIndex(id => element.id === this.savedPosts.id)
  this.savedPosts.splice(id, 1, newRating)
}
